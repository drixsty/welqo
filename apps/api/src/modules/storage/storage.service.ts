...
export class StorageService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    for (const bucket of buckets) {
      try {
        ...
        this.logger.error(
          `Error checking/creating bucket "${bucket}":`,
          error.message || error,
        );
      }
    }
  ...
  async uploadFile(
    ...
    try {
      ...
      this.logger.error(
        `Error uploading file to "${bucketName}":`,
        error.message || error,
      );
      throw error;
    }
  ...
  async getFileUrl(
    ...
    try {
      ...
      this.logger.error(
        `Error getting file URL from "${bucketName}":`,
        error.message || error,
      );
      throw error;
    }
  ...
  async getFileBuffer(bucketName: string, fileName: string): Promise<Buffer> {
    try {
      ...
      this.logger.error(
        `Error getting file buffer from "${bucketName}":`,
        error.message || error,
      );
      throw error;
    }
  ...
  async deleteFile(bucketName: string, fileName: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Error deleting file from "${bucketName}":`,
        error.message || error,
      );
      throw error;
    }
