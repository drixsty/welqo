...
import { ChevronDown, Zap, ArrowRight, Users, Calendar as CalendarIcon, Minus, Plus, ChevronLeft, ChevronRight, X } from "lucide-react";
    ...
    <div className="sticky top-24 bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-100 dark:border-white/5 shadow-sm transition-all">
      <div className="flex justify-between items-baseline mb-5">
        <div className="flex items-baseline gap-1">
          <span className="text-xl font-bold text-slate-900 dark:text-white">€{basePrice}</span>
              ...
              >
                <DayPicker
                  ...
                  classNames={{
                    ...
                    month: "space-y-2",
                    month_caption: "flex justify-between pt-1 relative items-center mb-1 px-1",
                    caption_label: "text-[11px] font-bold text-slate-900 dark:text-white capitalize",
                    nav: "flex items-center gap-0.5",
                    button_previous: "h-5 w-5 bg-transparent p-0 opacity-50 hover:opacity-100 transition-opacity flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-white/5",
                    ...
                    disabled: "text-slate-200 dark:text-slate-700 opacity-50 cursor-not-allowed line-through",
                    hidden: "invisible",
                  }}
                  components={{
                    Chevron: (props) => props.orientation === "left" ? <ChevronLeft className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />,
                  }}
                />
                <div className="mt-2 pt-2 border-t border-slate-50 dark:border-white/5 flex items-center justify-between">
                   <button onClick={() => setRange(undefined)} className="text-[9px] font-bold text-slate-400 hover:text-primary transition-colors flex items-center gap-1">
                     <X className="w-2.5 h-2.5" />
                     Effacer
                   </button>
                   <span className="text-[9px] font-medium text-slate-400 italic">
                      {nights > 0 ? `${nights} nuits sélectionnées` : "Sélectionnez vos dates"}
                   </span>
                </div>
              </motion.div>
            )}
            ...
            {isGuestOpen && (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-slate-900 dark:text-white">Adultes</p>
                    <p className="text-[9px] text-slate-400 font-medium">Max {maxGuests} voyageurs</p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold w-3 text-center">{guests}</span>
                    <button 
      ...
      <div className="space-y-2 mb-5 text-[11px]">
        <div className="flex justify-between">
          <span className="text-slate-500 font-medium">€{basePrice} x {nights} nuits</span>
          <span className="font-bold text-slate-900 dark:text-white">€{totalNights.toFixed(2)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-slate-500 font-medium">Frais de ménage</span>
          <span className="font-bold text-slate-900 dark:text-white">€{cleaningFee.toFixed(2)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-slate-500 font-medium">Taxes de séjour</span>
          <span className="font-bold text-slate-900 dark:text-white">€{totalTax.toFixed(2)}</span>
        </div>
