...
const QUARTIERS = [
  ...
  },
  {
    ...
    atout: "Ambiance unique, marché du dimanche, prix d'acquisition accessibles",
...
export function ArticleMeilleursQuartiers({ locale }: { locale: string }) {
  ...
  return (
    <article className="space-y-0">
      ...
      {/* Section 1 */}
      <h2 id="classement" className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-4 mb-4 scroll-mt-28">
        Notre classement des quartiers les plus rentables pour Airbnb à Lille
      </h2>
      <p className="text-slate-600 dark:text-slate-400 leading-relaxed mb-8">
        Classement basé sur les données du marché AirDNA, croisées avec nos performances
        internes sur les biens Welqo — taux d'occupation, revenu brut moyen T1 2025.
      </p>
      ...
      <div className="space-y-5 mb-12">
        {QUARTIERS.map((q) => (
          ...
          >
            {/* Top bar */}
            <div className={`${q.accentBg} px-5 py-3 flex items-center justify-between`}>
              <div className="flex items-center gap-3">
                <span className="text-white/50 text-xs font-black">{q.rank}</span>
                <h3 className="font-black text-white text-base tracking-tight">{q.name}</h3>
              </div>
              {/* Score bar */}
                  ...
                  {[...Array(5)].map((_, i) => (
                    <div
                      ...
                      className={`w-5 h-1.5 rounded-full ${
                        i < Math.round(q.score / 20) ? "bg-white" : "bg-white/20"
                      }`}
                    />
                  ))}
                </div>
                <span className="text-white text-xs font-black">{q.score}/100</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800 bg-slate-50 dark:bg-slate-900">
              {[
                ...
                <div key={label} className="px-4 py-3 text-center">
                  <p className="text-sm font-black text-slate-900 dark:text-white">{val}</p>
            ...
            <div className="p-5 bg-white dark:bg-slate-900 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="flex items-start gap-2.5 text-sm">
                ...
                <span className="text-slate-600 dark:text-slate-400">{q.limite}</span>
              </div>
            </div>
            <div className="px-5 pb-4 bg-white dark:bg-slate-900">
              <p className="text-xs text-slate-400 italic">Profil voyageur : {q.profil}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Section 2 */}
      <h2 id="choisir" className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-12 mb-6 scroll-mt-28">
        ...
        {[
          {
            ...
            detail: "Prix d'acquisition élevé, mais le revenu locatif compense largement. Vieux-Lille excelle sur les week-ends touristiques ; Euralille sur les déplacements d'affaires en semaine.",
          ...
          },
          {
            ...
            detail: "Meilleurs rapports prix d'achat / revenu locatif. Avec un appartement à 3 500 €/m², vous pouvez atteindre une rentabilité brute de 8 à 10 %.",
          ...
          <div key={title} className={`p-5 rounded-2xl border ${color}`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="font-black text-slate-900 dark:text-white text-sm">{title}</p>
            </div>
      ...
      {/* Section 3 */}
      <h2 id="gestion-pro" className="text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-12 mb-4 scroll-mt-28">
      ...
      <div className="my-6 p-6 bg-slate-950 dark:bg-slate-900 rounded-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="grid grid-cols-2 gap-4 mb-4">
            {[
              { label: "Gestion amateur",  val: "-20 à -30 %", sub: "de performances", color: "text-red-400" },
              { label: "Gestion Welqo",    val: "+38 %",       sub: "de revenus",      color: "text-emerald-400" },
            ].map(({ label, val, sub, color }) => (
              <div key={label} className="text-center p-4 bg-white/5 rounded-xl">
                <p className={`text-2xl font-black tracking-tight ${color}`}>{val}</p>
                <p className="text-slate-400 text-xs mt-1">{sub}</p>
                <p className="text-slate-500 text-[10px] mt-0.5 font-black uppercase tracking-wide">{label}</p>
              </div>
            ))}
          </div>
          <p className="text-slate-400 text-sm leading-relaxed">
            Ces données correspondent à des biens gérés de manière optimale. En gestion amateur,
            attendez-vous à des performances 20 à 30 % inférieures : annonces sous-optimisées,
            tarification statique, communication lente.
          </p>
        </div>
      </div>

      <p className="text-slate-600 dark:text-slate-400 leading-relaxed mt-6">
        Quel que soit le quartier que vous choisissez, une gestion professionnelle peut faire la
        différence entre un bien rentable et un bien qui performe véritablement.{" "}
